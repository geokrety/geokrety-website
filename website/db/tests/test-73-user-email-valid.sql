-- Start transaction and plan the tests.

BEGIN;

-- SELECT * FROM no_plan();
SELECT plan(8);

-- const USER_EMAIL_NO_ERROR = 0;
SELECT lives_ok($$INSERT INTO "gk_users" ("id", "username", "registration_ip", "email_invalid") VALUES (1, 'username1', '127.0.0.1', 0::smallint)$$);
-- const USER_EMAIL_DOES_NOT_EXIST = 1;
SELECT lives_ok($$INSERT INTO "gk_users" ("id", "username", "registration_ip", "email_invalid") VALUES (2, 'username2', '127.0.0.1', 1::smallint)$$);
-- const USER_EMAIL_UNCONFIRMED = 2;
SELECT lives_ok($$INSERT INTO "gk_users" ("id", "username", "registration_ip", "email_invalid") VALUES (3, 'username3', '127.0.0.1', 2::smallint)$$);
-- const USER_EMAIL_MISSING = 3;
SELECT lives_ok($$INSERT INTO "gk_users" ("id", "username", "registration_ip", "email_invalid") VALUES (4, 'username4', '127.0.0.1', 3::smallint)$$);
-- const USER_EMAIL_MAILBOX_FULL = 4;
SELECT lives_ok($$INSERT INTO "gk_users" ("id", "username", "registration_ip", "email_invalid") VALUES (5, 'username5', '127.0.0.1', 4::smallint)$$);
-- const USER_EMAIL_DETECTED_AS_SPAM = 5;
SELECT lives_ok($$INSERT INTO "gk_users" ("id", "username", "registration_ip", "email_invalid") VALUES (6, 'username6', '127.0.0.1', 5::smallint)$$);

SELECT throws_ok($$INSERT INTO "gk_users" ("id", "username", "registration_ip", "email_invalid") VALUES (7, 'username7', '127.0.0.1', 6::smallint)$$);
SELECT throws_ok($$INSERT INTO "gk_users" ("id", "username", "registration_ip", "email_invalid") VALUES (8, 'username8', '127.0.0.1', 7::smallint)$$);

-- Finish the tests and clean up.
SELECT * FROM finish();
ROLLBACK;
;
