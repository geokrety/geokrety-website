...
EOD;
    ...
    $sql = <<<EOQUERY
SELECT  DATE(`date`) as date, `tresc` as content, `tytul` as title, `who`,
        `userid`, `komentarze` as comment_count, `news_id`
FROM    `gk-news`
ORDER   BY `date` DESC
LIMIT   2
EOQUERY;
    ...
    $sql = <<<EOQUERY
SELECT ru.ruch_id, ru.id, ru.lat, ru.lon, ru.country, ru.waypoint, ru.droga,
            ru.data, ru.user, ru.koment, ru.logtype, ru.username, us.user,
            gk.nazwa, gk.typ, gk.owner, pic.plik, ru.zdjecia
...
EOQUERY;
    ...
    $sql = <<<EOQUERY
SELECT    `gk-geokrety`.id, nr as tracking_code,
...
EOQUERY;
    ...
    $sql = <<<EOQUERY
SELECT  user as username, userid as user_id
FROM    `gk-users`
WHERE   ostatni_login > DATE_SUB(NOW(), INTERVAL 5 MINUTE)
EOQUERY;
    $result = mysqli_query($link, $sql);
