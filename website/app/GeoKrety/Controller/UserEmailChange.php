...
class UserEmailChange extends Base {
    public function get($f3) {
        ...
        if ($f3->exists('POST.token')) {
            $token->load(array('token = ? AND used = ? AND DATE_ADD(created_on_datetime, INTERVAL ? DAY) >= NOW()', $f3->get('POST.token'), EmailActivation::TOKEN_UNUSED, GK_SITE_EMAIL_ACTIVATION_CODE_DAYS_VALIDITY));
            if ($token->valid()) {
