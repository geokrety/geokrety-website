...
.imgup3{
	...
	margin-right:auto;
        ...
        $TRESC .= '<form name="imgup" method="post" enctype="multipart/form-data" action="'.$_SERVER['PHP_SELF'].'?typ='.$g_typ.'&amp;id='.$g_id.($rename ? '&amp;rename='.$g_rename : '').($multiphoto ? '&amp;all_id='.$all_id : '').'">'.
            ...
            (($can_set_avatar and $allowed_to_upload and !$editing_mode) ? ' <tr><td><div style="text-align:right">'._('Use as avatar').': </div></td><td><input id="avatar" type="checkbox" name="avatar" value="true" '.(isset($_GET['avatar']) ? 'checked' : '').' /><span class="bardzomale"> ('.sprintf(_('Geokret\'s main picture, displayed under this icon: %s'), '<img src="'.CONFIG_CDN_ICONS.'/idcard.png" alt="avatar"/>').')</span></td></tr>' : '').
